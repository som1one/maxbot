...
async def send_start_menu(target, user_id: int):
    ...
    await target.send(
        "Здравствуйте!\n"
        "Я бот компании «СБ Карго». Помогаем юридическим лицам с международной перевозкой грузов и таможенным оформлением «под ключ».\n\n"
        "Продолжая общение в этом чате, вы даете согласие на обработку персональных данных. "
        "https://bit.sbcargo.ru/pub/imol.php?id=1&sec=af5bok&user_lang=ru\n\n"
        "Чтобы быстрее подсказать по вашему запросу, выберите, что актуально:",
        keyboard=keyboard
    )
